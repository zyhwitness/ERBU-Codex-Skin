...
function svgDimensions(bytes) {
  ...
  const width = parseLength(attrValue("width"));
  const height = parseLength(attrValue("height"));
  if (width && height) return { width, height };
  const viewBox = attrValue("viewBox");
  if (!viewBox) return null;
  const parts = viewBox.trim().split(/[\s,]+/).map(Number);
  if (parts.length !== 4 || parts.some((value) => !Number.isFinite(value))) return null;
  const vbWidth = Math.round(parts[2]);
  const vbHeight = Math.round(parts[3]);
  return vbWidth > 0 && vbHeight > 0 ? { width: vbWidth, height: vbHeight } : null;
}
...
export function readImageMetadata(value, extension = "") {
  ...
  else if (normalized === ".svg" || ascii(bytes, 0, 4).toLowerCase() === "<svg") dimensions = svgDimensions(bytes);
